...
import { updateArchitectProfile, uploadArchitectImage } from "@/app/actions/architect";
...
/**
 * Image field for the block property panel: paste a URL or upload a file.
 * Uploads reuse the existing `uploadArchitectImage` action (cms-images bucket,
 * server-side type/size validation), the same path the branding fields use.
 */
    ...
    <div className="mt-1 space-y-2">
      <div className="flex gap-2">
        ...
        <button
          ...
          disabled={uploading}
          className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-2 text-xs font-semibold text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-50"
        >
      ...
      {value ? (
        <div className="relative h-24 w-full overflow-hidden rounded-lg border border-gray-200 bg-gray-50">
          ...
          <button
            type="button"
            onClick={() => onChange("")}
            className="absolute right-1.5 top-1.5 rounded-md bg-white/90 px-2 py-1 text-[10px] font-bold text-gray-700 shadow-sm hover:bg-white"
          >
            Remove
          </button>
        </div>
      ) : (
        <p className="text-[11px] text-gray-400">JPEG, PNG, WebP or GIF, up to 5MB.</p>
      )}
    </div>
  );
}
              ...
              <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                ...
                <div className="space-y-3">
                  {selectedDef.fields.map((field) => {
                    ...
                    // Not wrapped in <label>: the uploader has its own controls,
                    // and a nested file input would be triggered by the label.
                    if (field.type === "image") {
                      return (
                        <div key={field.key}>
                          ...
                          <BlockImageField
                            value={value}
                            onChange={(url) => updateField(selected.id, field.key, url)}
                          />
                          {field.hint ? (
