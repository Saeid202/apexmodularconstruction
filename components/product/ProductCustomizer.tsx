      ...
      {groups.map((group, idx) => (
        <div key={group.id} className="group/category">
          <div className="flex items-center gap-3 mb-5">
            <span
              className="flex h-7 w-7 items-center justify-center rounded-lg text-xs font-black text-white shadow-lg"
          ...
          <div className="flex flex-col gap-3">
            {group.options.map((option) => {
              const isMultiSelect =
                group.visual_type === 'wall-color' || group.name.toLowerCase().includes('color')
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div className="min-w-0">
                        <h4
                          className={`text-base font-black tracking-tight transition-colors ${isSelected ? 'text-purple-900' : 'text-gray-900'}`}
                        >
                          {displayName}
                        </h4>
                        {colorValue ? (
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        <div className="flex flex-col items-end">
                          <span className="text-lg font-black" style={{ color: PURPLE }}>
                            +$
                            {option.price_modifier.toLocaleString(undefined, {
      ...
      {groups.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-gray-500 max-w-xs mx-auto">
            This product does not have any specific customization options yet.
          </p>
        </div>
      )}
    </div>
